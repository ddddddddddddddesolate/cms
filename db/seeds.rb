...
2.times do |e|
  ...
  2.times do |d|
    playlist_user1 = Playlist.create(name: "Playlist for display ##{d}", user_id: user1.id, duration: rand(3600..36000))
    playlist_user2 = Playlist.create(name: "Playlist for display ##{d}", user_id: user2.id, duration: rand(3600..36000))
    ...
    3.times do |s|
      ...
      slide_user1 = Slide.create(playlist_id: playlist_user1.id, content_id: content_user1.id, weight: s)
      slide_user2 = Slide.create(playlist_id: playlist_user2.id, content_id: content_user2.id, weight: s)
    end
  end
end
